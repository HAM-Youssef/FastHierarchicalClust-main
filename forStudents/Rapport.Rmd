...
```{r}
naive_hclust(generate_dist_matrix(5))
...
## Simulations qui comparent les temps entre le naïf et le nouvel algo vs version C++
...
fast_hclust_cpp(generate_dist_matrix(5), method = "single")
```
...
# Ajuster un modèle t = k * n^2
fit_R <- lm(time ~ I(n^3) - 1, data = filter(results, method == "fast_hclust_R"))
fit_Rcpp <- lm(time ~ I(n^3) - 1, data = filter(results, method == "fast_hclust_Rcpp"))
...
- \( k \) pour `fast`_hclust_R` :  
  $$
  k = 2{.}341189 \times 10^{-7}
  $$

- \( k \) pour `fast_hclust_Rcpp` :  
  $$
  k = 3{.}681993 \times 10^{-9}
  $$
...
Cela montre que l'implémentation en C++ peut traiter des tailles \( n \) environ **8 fois plus grandes** que celle en R dans le même temps imparti
...
- R pur (`naive_hclust_R`) :  
  ...
  n \leq 642
  $$
  Cela montre que l'implémentation en R de l'algo optimisé peut traiter des tailles \( n \) environ ***1{,}5 fois plus grandes*** que celle en R dans le même temps imparti
